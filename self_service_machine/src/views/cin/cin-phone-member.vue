    ...
    <div class="masking">
      ...
      <div class="phone" v-show="showKeyboard" @clickoutside="closeModal">
        ...
        <div class="inputField">
          <input type="text" placeholder="请输入您的手机号" :value="clavier">
    ...
    methods: {
      ...
      inquire:function(){
        ...
        if(!(/^1[3456789]\d{9}$/.test(that.clavier))){
          ...
          that.$axios({
            method : 'post',
            url : that.api.api_price + '/v1/customer/machine/search_card_by_info/',
            data : {
              search_type : 2,
              info : that.clavier //这里是手机号
            }
          }).then((res)=>{
            console.info(res);
            if(res.data.message === 'success'){
              ...
              that.$router.push({path:'/cin-nores'});// 存在会员信息
            }else{
              //如果没有预定信息的时候直接提示不存在用户信息
              instance('该手机号没有查到会员信息')
              //that.hintInfo('warning','该手机号没有查到会员信息');
              //that.handleSpeak('该手机号没有查到会员信息')
            }
          }).catch((err)=>{
            console.error(err);
          })
          console.log(that.clavier);
        }
